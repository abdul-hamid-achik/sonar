...
func TestAutoCommandAssessmentAllowsExactMinervaWorkspaceQueries(t *testing.T) {
	...
	for _, command := range []string{
		...
		"./bin/minerva learn",
		"./bin/minerva learn --json",
		"./bin/minerva status --json",
		"./bin/minerva library lint",
		"./bin/minerva library lint --json",
		"./bin/minerva skill show qa-tester",
		"./bin/minerva skill resolve review",
		"./bin/minerva skill resolve review --json",
...
func TestPackageRunScriptsAreScopedToLocalVerification(t *testing.T) {
	...
	// Classification is about script names, not about which package managers
	// happen to be installed. CI runners have npm but not pnpm or bun; without
	// host-owned stubs LookPath refuses those executables before the script
	// name is ever read, and the test fails for ambient PATH rather than policy.
	hostBin := t.TempDir()
